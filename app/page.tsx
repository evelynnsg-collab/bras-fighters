...
const fighters = [
  ...
  { id: "evelyn", name: "Evelyn S.", style: "Força e resistência", special: "Golpe de Comando", side: "left" },
] as const;
...
export default function Home() {
  ...
  const animate = (index: 0 | 1, move: string) => {
    ...
    window.setTimeout(() => setMoves(v => index === 0 ? ["idle", v[1]] : [v[0], "idle"]), move === "special" ? 1450 : move === "kick" ? 560 : 470);
  };
  ...
  return <main className="game-shell">
    ...
    {screen === "fight" && <section className="fight-wrap"><div className="hud"><FighterHud name={p1Name} hp={hp[0]} wins={wins[0]} side="p1" fighterId={fighters[p1Choice].id}/><div className="clock">{String(time).padStart(2,"0")}<small>ROUND {wins[0]+wins[1]+1}</small></div><FighterHud name={p2Name} hp={hp[1]} wins={wins[1]} side="p2" fighterId={fighters[p2Choice].id}/></div>
      <div className={`arena arena-${arena} ${moves[1]==="hit"?"impact-shake":""} ${moves[0]==="special"?"evelyn-special-impact":""}`}><div className="stage-depth"/><div className="moving-train"><i/><i/><i/><i/><i/></div><div className="arena-name">FASE {stage+1}/5 • ESTAÇÃO {stages[stage].name.toUpperCase()}</div><div className={`fighter player fighter-${fighters[p1Choice].id}-actor ${moves[0]}`} style={{left:`${position[0]}%`,bottom:`${verticalPosition[0]}%`}}><div className={`sprite fighter-${fighters[p1Choice].id}`}>{fighters[p1Choice].id==="evelyn"&&<><i className="ev-limb ev-upper-arm"/><i className="ev-limb ev-forearm"/><i className="ev-limb ev-thigh"/><i className="ev-limb ev-shin"/><i className="ev-baton"/><i className="ev-impact"/></>}</div><span>{p1Name}</span></div><div className={`fighter cpu ${moves[1]}`} style={{left:`${position[1]}%`,bottom:`${verticalPosition[1]}%`}}><div className={`sprite fighter-${fighters[p2Choice].id}`}/><span>{p2Name}</span></div>{(moves[0]==="punch"||moves[0]==="kick"||moves[0]==="special")&&<div className={`hit-spark spark-${moves[0]}`}/>}<div className="fight-message">{message}</div>{combo>=2&&<div className="combo-badge">{combo}<small>HIT COMBO</small></div>}<div className="floor-glow"/></div>
